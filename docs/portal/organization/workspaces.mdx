...
The following table describes whether a feature is supported in the context of multiple workspaces.
How the feature is supported can vary, so be sure to refer to the preceding descriptions for more details.

:::note
Most features marked as unsupported are still available but are currently limited to a specific workspace and unable to share instances or configurations across linked workspaces.
:::

| Feature                                 | Supported with multiple workspaces |
| :-------------------------------------- | :--------------------------------: |
| [User management](#user-management)     | <Check alt="Yes" />                |
| [Single sign-on (SSO)](#single-sign-on) | <Check alt="Yes" />                |
| Reports                                 | <Close alt="No" />                 |
| SIM inventory                           | <Close alt="No" />                 |
| Policy management                       | <Close alt="No" />                 |
| Cloud Connect                           | <Close alt="No" />                 |
| Virtual private networks (VPNs)         | <Close alt="No" />                 |
| User logs for auditing                  | <Close alt="No" />                 |
| Centralized billing                     | <Close alt="No" />                 |
| Split billing and reporting             | <Close alt="No" />                 |
...
When emnify sets up your multiple workspace architecture, a [**SuperAdmin**](#superadmin-user-role) role is also provided.

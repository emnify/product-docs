...
There are two types of policies, [**Service Policies**](#service-policies) and [**Coverage Policies**](#coverage-policies).
...
| **SMS** | - SMS MO (devices may send SMS) & sending limit </br> - SMS MT (devices may receive SMS) & receiving limit </br> - allow/deny the exchange of SMS <ul><li>between devices</li><li>with external parties</li></ul> |
...
For a single device, you can select its new policy from a dropdown list when viewing the **Device Details** by selecting **Edit** as described in the [Single device assignment](/portal/device-policies/how-to-assign-policy) section.

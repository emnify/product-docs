...
3. Find the device you want to manage using the SMS console and click **Details**.
4. Select **+ Add tag** under the device name.
5. Tag the devices using the following syntax:

```txt
sms-UNIQUE_IDENTIFIER
```

Replace _`UNIQUE_IDENTIFIER`_ with a unique string of  letters and numbers to identify the tagged devices. **Minimum 5** characters are required.
...
Instead, use a long and random string of **minimum 5** numbers and letters.
